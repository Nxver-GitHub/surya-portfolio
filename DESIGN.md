...
The home landscape uses an uncropped 4:3 coordinate plane capped at 1440px. Desktop width also responds to viewport height when width is at least 1100px and height at least 700px, deriving the map from the space left over after the chrome: `(100dvh - 400px) * 4 / 3`. That 400px is the header, information strip, control strip and HUD totals added up — 354px — plus the 46px the Sound Select bar reserves along the bottom edge. **If the header's height changes, or the bar's, this constant has to change with it**, or the HUD drops below the fold and the desktop screen starts to scroll. The `.console-options` right-edge offset is derived from the same expression and has to move with it.

The no-scroll guarantee is bounded, and honestly so: a `min-width: 720px` floor stops the map shrinking past legibility, which means it holds on viewports about **980px tall and up**. Shorter desktop windows scroll by a few pixels, and shorter still by more. That is the floor doing its job rather than a bug to chase — closing the gap would mean shrinking the map below a usable size. The chrome also breathes slightly with width, because the information strip's caption wraps to a second line on narrow screens, so the constant is a close fit rather than an exact one.
...
- **Sound Select bar:** [SoundBar.tsx](src/components/sound/SoundBar.tsx) is the music deck — a 46px strip fixed along the bottom edge of every screen, mounted once in the root layout. Its top border is the same stamped 2px rule as the map's control strip, because that is what a strip along the bottom of a screen already looks like here. Transport keys and the track readout hold the left at the shared 44px control height, the CC BY 4.0 credit the right; the rotation and level popup opens upward. `body` reserves the bar's height as padding, so no page hides beneath it. Phones fold the credit into the popup and drop the prev key, keeping the thumb zone to play/pause, next and a truncated title.
