...
- PS1 flavor: flat/Lambert (Gouraud-like) shading, low-poly geometry, indexed-color/low-res textures, optional dithering. Live scenes render through `ConsoleResolution` (`src/components/gt/ConsoleResolution.tsx`), which caps the WebGL framebuffer at 480×360 while preserving aspect ratio; DOM overlays stay at native resolution.
- The era reaches the 2D UI through *lettering and control chrome* — Pixelify Sans labels, stamped bevels, amber selection — but **never through the reading layer**. Prose, links, spec sheets and source photography stay crisp and independently rendered at native resolution.
...
> **`DESIGN.md` at the repo root is the source of truth for the visual system** — palette, bevel ramps, typography, component vocabulary. It is captured from the shipped implementation. Read it before changing anything visual; the bullets here are the standing law, not a second palette.

- Base look: **original-console racing frontend** — a late-1990s console game's menu system. Midnight navy `#0b101b` ground, amber `#ffbf19` as the system/selection color, red `#d82c35` for title rules. Controls are *stamped, never flat*: a light top/left bevel, a dark bottom/right bevel and an inset keyline (the ramps are enumerated in `DESIGN.md`). The component vocabulary lives in `globals.css` under `@layer components` — `.gt-title` + `.gt-rule` (heavy drop-shadowed serif title over a short red rule), `.plate` / `.plate-hot` (stamped panel buttons), `.lozenge` (square steel back-button/chip), `.badge-rim` + `.badge-face` (enamel badge: colored rim, flat enamel face — see `LicenseBadge` and `GtMark`), `.ts-hard` (hard black text drop shadow) — with the console layer in `src/app/console.css` layered on top. Square corners throughout. Never flat bright color fields as page backgrounds (an earlier flat-yellow version was explicitly rejected as "flash game" slop).
  - Superseded in the 2026-09 redesign, so you don't try to restore it: the former GT2 orange `#EF8100` on `#0a0a0b`, the global grid-paper texture, the `.gt-rule` diagonal kick, and the orange `.gt-crumb` strip. Those are **intentionally gone**.
  - Period graphics belong to *scenes and selected interface roles*. **Never rasterize prose, links or source photography** to imitate the era — the Readable Evidence Rule in `DESIGN.md`.
  - Intentional period features that generic "AI-slop" detectors misread: triangular CSS selection cursors (`width:0;height:0` + transparent borders) and hard bevels. Do not remove them because a detector flagged a border triangle.
- Liveries: components also carry **livery-inspired decal treatments** (`content/liveries.ts`, `LiveryStripe`, badge rim colors) — Marlboro, Gulf, West, Jägermeister, Leyton House, Warsteiner, Red Bull color systems. One livery per pavilion, on card/chip chrome only. **No real brand logos or wordmarks** — colors and patterns only (trademark safety on a public repo/site). This includes the Gran Turismo emblem and logotype: the site's own mark is the enamel "SP" badge (`GtMark`), built from project fonts, never a GT logo or a rip of its typeface.
- Typography: Source Serif 4 (900) for page titles (GT2 headers were drop-shadowed serifs), **Pixelify Sans** (self-hosted, 400–700) as `--font-display` for navigation, labels and map controls, Satoshi (self-hosted) for body, Saira for numeric readouts and small display text. No external font CDNs (CSP). Saira is no longer condensed site-wide (`font-stretch: normal`).
